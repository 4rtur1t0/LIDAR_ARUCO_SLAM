...
class LoopClosing2():
    ...
    def find_feasible_triplets(self):
        ...
        for i in range(0, len(positions), step_index):
            ...
            triplets = self.find_j_k_within_radii(tree=tree, positions=positions, i=i)
            triplets_global.append(triplets)
        # flatten
        triplets_global = [item for sublist in triplets_global for item in sublist]
        return triplets_global

    def find_j_k_within_radii(self, tree, positions, i):
        """
        ...
        r3 = 2.0
        max_diff_index = 100
        random_choice_sample_far = 2
        # Find all nearby poses within the search radii r1 and r2
        neighbors_in_r1 = tree.query_ball_point(positions[i, :2], r1)
        neighbors_in_r2 = tree.query_ball_point(positions[i, :2], r2)
        # find loopclosings within radius r3
        neighbors_in_r3 = tree.query_ball_point(positions[i, :2], r3)
        # neighbors = neigbors_in_rmax not in neighbours_in_rmin
        neighbors_r1_r2 = np.setdiff1d(neighbors_in_r2, neighbors_in_r1)
        diff_indices = neighbors_r1_r2-i
        # the neighbor index must be close to i and positive (looking from i=0 to i=N
        neighbors_close_in_index = np.where((diff_indices > 0) & (diff_indices < max_diff_index))[0]
        if len(neighbors_close_in_index) > 0 and len(neighbors_r1_r2) > 0:
            # return the first result, the closest index to i (j) within the specified radii
            j = neighbors_close_in_index[0]
            j = neighbors_r1_r2[j]
        else:
            return []
        if len(neighbors_in_r3) == 0:
            return []
        # Now look for positions within r3 (asume that i and j are close in distance).
        # look for indices that are ahead of i
        diff_indices = np.array(neighbors_in_r3) - i
        neighbors_far_index = np.where(diff_indices > max_diff_index)[0]
        num_samples = min(random_choice_sample_far, len(neighbors_far_index))
        neighbors_far_index = np.random.choice(neighbors_far_index, num_samples, replace=False)
        triplets = []
        # choose randomly from neighbors_far_index
        for k in neighbors_far_index:
            k = neighbors_in_r3[k]
    ...
    def check_triplet_transforms(self, graphslam, triplet_transforms):
        ...
        for triplet_transform in triplet_transforms:
            ...
            i = triplet_transform[0]
            j = triplet_transform[1]
            k = triplet_transform[2]
            Tik = triplet_transform[3]
            Tjk = triplet_transform[4]
            # get the current estimate for the consecutive i and j indices
            Tij = graphslam.get_relative_solution_transform(i=i, j=j)
            # CAUTION: Tij is in the reference system of the GPS
            # Tjk and Tik are computed from the registartion in the lidar coordinate system--> must be transformed
            Tik = Tik * graphslam.T0_gps
            Tjk = Tjk * graphslam.T0_gps
            I=Tij*Tjk*Tik.inv()
            ...
            if self.check_distances(I):
                ...
                print(10 * '#')
                result_triplet_transforms.append([i, j, k, Tik, Tjk])
    ...
    # def loop_closing_simple(self, graphslam, lidarscanarray, triplets):
    #     """
    ...
    #     while self.graphslam.current_estimate.exists(X(i)):
    #         ce = self.graphslam.current_estimate.atPose3(X(i))
